...
# def poly_wbf(polygons, threshold):
#     assert isinstance(polygons, list)
#
#     polygons = np.array(sorted(polygons, key=lambda x: x[-1]))
#
...
#             B = polygons[index[i]][:-1]
#             iou_list[i] = boundary_iou(A, B)
#         cluster_index = np.where(iou_list > threshold)
#         index = np.delete(index, cluster_index)
#         cluster_polygons = np.delete(polygons, index, axis=0)
#         score_num = np.sum(cluster_polygons[:, -1:])
#         new_polygons = np.matmul(cluster_polygons[:, :-1].transpose(), cluster_polygons[:, -1:]) / score_num
#         new_polygons.astype(np.int32)
#         new_polygons = np.row_stack((new_polygons, score_num / cluster_polygons.shape[0]))
#         keep_poly = keep_poly + new_polygons.transpose().tolist()
#         # return 0
#     return keep_poly
