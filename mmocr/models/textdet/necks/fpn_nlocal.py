...
class BasicBlock(nn.Sequential):
    def __init__(
            ...
            bn=False, pooling=False, act=nn.PReLU()):
...
class FPN_NLOCAL(nn.Module):
    ...
    def __init__(self,
        ...
        super(FPN_NLOCAL, self).__init__()
        ...
        self.nlocal = NonLocalAttention(channel=256)
...
#         # middle_shape = laterals[1].shape[2:]
#         # top_shape = laterals[2].shape[2:]
#         for i in range(1, used_backbone_levels):
#             middle += F.interpolate(
#                 laterals[i], size=bottom_shape, mode='nearest')

#         middle = self.nlcoal(middle)

#         laterals[0] = middle
#         for i in range(1, used_backbone_levels):
#           laterals[i] = F.interpolate(
#                 middle, size=laterals[i].shape[2:], mode='nearest')
        ...
        # Non-Local Attention
#         middle = outs[1]
#         middle_shape = outs[1].shape[2:]
#         # middle_shape = laterals[1].shape[2:]
#         # top_shape = laterals[2].shape[2:]
#         for i in range(1, used_backbone_levels):
#             middle += F.interpolate(
#                 outs[i], size=middle_shape, mode='nearest')

#         middle = self.nlocal(middle)

#         outs[0] = middle
#         for i in range(1, used_backbone_levels):
#             outs[i] = F.interpolate(
#                 middle, size=outs[i].shape[2:], mode='nearest')

        middle = outs[1]
        middle_shape = outs[1].shape[2:]
        middle += F.interpolate(outs[0], size=middle_shape, mode='nearest')+F.interpolate(outs[2], size=middle_shape, mode='nearest')
        
        middle = self.nlocal(middle)
        
        outs[1] = middle
        outs[0] = F.interpolate(middle, size=outs[0].shape[2:], mode='nearest')
        outs[2] = F.interpolate(middle, size=outs[2].shape[2:], mode='nearest')
