...
class MSROCALoss(nn.Module):
    ...
    def forward(self, preds, _, p3_maps, p4_maps, p5_maps):
        ...
        loss_hor = torch.tensor(0., device=device).float()
        loss_ver = torch.tensor(0., device=device).float()
    ...
    def forward_single(self, pred, gt):
        ...
        hor_pred = pred[2].permute(0, 2, 3, 1).contiguous()
        ver_pred = pred[3].permute(0, 2, 3, 1).contiguous()
        ...
        hor_pred = hor_pred.view(-1)
        ver_pred = ver_pred.view(-1)
        ...
        loss_hor_bce = self.balanced_bce_loss(hor_pred, tr_mask.long(), train_mask.long())
        loss_hor = 0.01 * loss_hor_bce + self.dice_loss(tr_mask.long(), hor_pred)

        loss_ver_bce = self.balanced_bce_loss(ver_pred, tr_mask.long(), train_mask.long())
        loss_ver = 0.01 * loss_ver_bce + self.dice_loss(tr_mask.long(), ver_pred)

        return loss_tr, loss_tcl, loss_reg_x, loss_reg_y, loss_hor, loss_ver
